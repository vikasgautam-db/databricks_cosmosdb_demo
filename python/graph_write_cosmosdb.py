...
# MAGIC This notebook is based on the `GraphFrames` example [specified here](https://graphframes.github.io/user-guide.html#tab_python_0). It requires [graphframes](https://spark-packages.org/package/graphframes/graphframes) and [azure-cosmosdb-spark (uber jar)](https://github.com/Azure/azure-cosmosdb-spark#using-databricks-notebooks) libraries to be uploaded and attached to the cluster.
...
cosmosDbVertices = to_cosmosdb_vertices(g.vertices, "entity", "name")
...
cosmosDbEdges = to_cosmosdb_edges(g, "relationship", "name")
...
cosmosDbConfig = {
  ...
  "spark.cosmos.container" : "persons",
  "spark.cosmos.write.strategy" : "ItemOverwrite"
}

cosmosDbFormat = "com.microsoft.azure.cosmosdb.spark"

cosmosDbVertices.write.format("cosmos.oltp").mode("append").options(**cosmosDbConfig).save()
cosmosDbEdges.write.format("cosmos.oltp").mode("append").options(**cosmosDbConfig).save()
